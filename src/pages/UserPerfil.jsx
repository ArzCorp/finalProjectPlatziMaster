import React, { PureComponent } from 'react';
import { connect } from 'react-redux';

import * as userActions from '../actions/userActions';
import * as buttonsActions from '../actions/ButtonsActions';
import * as modalActions from '../actions/ModalActions';

import ImageProfile from '../components/atoms/ImageProfile';
import NavBar from '../components/organisms/NavBar';
import SettingsNav from '../components/organisms/SettingsNav';
import Settings from '../components/organisms/SettingsClothes';

class UserPerfil extends PureComponent {
  render() {
    const {
      buttonsReducers: { isActive },
      modalReducers: { isOpenModalImage, isOpenModalAdd, isOpenModalEdit },
      pullDisable,
      pullActive,
      isOpen,
      isClose,
    } = this.props;

    return (
      <section className="userprofile__entry">
        <NavBar />
        <ImageProfile
          url="data:image/jpeg;base64,"
          isOpen={isOpen}
          isClose={isClose}
          isOpenModal={isOpenModalImage}
        />
        <SettingsNav
          isActive={isActive}
          pullActive={pullActive}
          pullDisable={pullDisable}
        />
        <Settings
          isActive={isActive}
          pullDisable={pullDisable}
          isOpenModalAdd={isOpenModalAdd}
          isOpenModalEdit={isOpenModalEdit}
          isOpen={isOpen}
          isClose={isClose}
        />
      </section>
    );
  }
}

const mapStateToProps = ({ userReducer, buttonsReducers, modalReducers }) => ({
  userReducer,
  buttonsReducers,
  modalReducers,
});

const mapDipatchToProps = {
  ...userActions,
  ...buttonsActions,
  ...modalActions,
};

export default connect(mapStateToProps, mapDipatchToProps)(UserPerfil);
